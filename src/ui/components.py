...
def create_metric_card_with_tooltip(title, value, tooltip_text="", text_color=None):
    ...
    # Create a unique ID for the tooltip target
    tooltip_id = {"type": "tooltip", "index": title.replace(" ", "-").lower()}
    
    # Return a single component (html.Div) that contains both elements
    return html.Div([
        # The card
        dbc.Card([
            dbc.CardBody([
    ...
    ])  # Wrapper div to contain both components
